...
namespace POPBlocks.Scripts
{
    ...
    public class LevelManager : MonoBehaviour
    {
        ...
        //regenerate level object if no matches on the field
        ...
        private IEnumerator RegenLevel()
        {
            ...
            {
                i.SqueezeAnimation(() => i.colorComponent[0].RandomizeColor(new int[0]), () =>
                {
        ...
        public bool bonusAppearAnimate;
        ...
        public void ActivateItem(Item itemMatching)
        {
            ...
            if (GameManager.Instance.boostTypesGame.Any())
            {
                ...
                BoostButton.ProceedBoost(boost, itemMatching);
        ...
        IEnumerator ActivateItemAndMatch(Item itemMatching)
        {
            ...
            if (itemMatching.matchable)
            {
                ...
                if (itemMatchingBonusNum > 0)
                    bonusAppearAnimate = true;
                foreach (var item in matchedItems)
                {
                    if (!item) continue;

                    // var itemsAround = Field.Instance.GetItemsCross(item);
                    // itemsAround.ForEachY(i => i.DestroyObstacles(item));
                    if (itemMatchingBonusNum == 0)
                    {
                    ...
                    {
                        item.transform.DOMove(itemMatching.transform.position, animTime).OnComplete(() => { item.DestroyItemStart(false, 0,noEffect: true); });
                    }
                }
                ...
                if (itemMatchingBonusNum > 0 && itemMatchingBonusNum < bonusMatch.BonusMatches.Length)
                {
                    ...
                    // SoundBase.Instance.PlayOneShot(SoundBase.Instance.bonusAppear);
            ...
            yield return new WaitUntil(() => counterDelay >= destroyingBonusesList.Count);
            // for (var index = 0; index < destroyingBonusesList.Count; index++)
            // {
            //     var item = destroyingBonusesList[index];
            //     if (!item) continue;
        ...
        public void CheckMovesAndTargets()
        {
            ...
            if (GameState != GameState.Playing || (GameObject.FindWithTag("AnimatedTarget") && !targetFinished)) return;
            if (targetFinished)
                GameState = GameState.PreWin;
            else if (moves.GetValue() <= 0)
            {
