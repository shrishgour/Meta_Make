...
namespace POPBlocks.Server.Network
{
    ...
    public class NetworkFriendsManager
    {
            ...
            if (friendsManager != null)
            {
                friendsManager.PlaceFriendsPositionsOnMap(list =>
                {
                    foreach (var item in list)
                    {
                        ...
                        FriendData friend = FacebookManager.Friends.Find(bk =>
                            bk.userID == item.Key && bk.userID != NetworkManager.UserID);
                        if (friend != null)
                        {
                            friend.level = item.Value;
                        }
        ...
        {
            // GameObject.FindObjectOfType<LevelManager>().THIS.StartCoroutine(GetLeadboardCor()));
        }
