    ...
    public class LevelsMap : Levels {
        ...
        public static List<MapLevel> GetMapLevels()
        {
            ...
            if (MapLevels.Count == 0)//1.4.4
                MapLevels = FindObjectsOfType<MapLevel>().OrderBy(ml => ml.Number).Where(i=>i!=null).ToList();
        ...
        public void Reset()
        {
            ...
            if (number > 1 && PlayerPrefs.GetInt("win")==1)
            {
        ...
        internal static void OnLevelSelected(int number)
        {
            if ( !IsLevelLocked(number))
                LevelSelected_(_instance, number);
        ...
        public void SetStarsEnabled(bool bEnabled)
        {
            ...
            int starsCount = 0;
            foreach (MapLevel mapLevel in GetMapLevels().Where(i=>i!=null))
            {
