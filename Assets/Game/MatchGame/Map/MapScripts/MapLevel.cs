    ...
    public class MapLevel : MonoBehaviour
    {
        ...
        public void UpdateState(int starsCount, bool isLocked)
        {
            ...
            if (IsLocked)
            {
                ...
                starsHolder.ToList().ForEach(i => i.SetActive(false));
            }
            else
            {
        ...
        {
            FindObjectsOfType<IdleCircleMapEffect>().ToList().ForEach(x=>Destroy(x.gameObject));
            var i = Instantiate(idleEffect, transform.position, Quaternion.identity, transform);
            i.transform.localScale = new Vector3(1f,1,1);
        }
    }
}
